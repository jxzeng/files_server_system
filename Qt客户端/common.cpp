    ...
    if (error.error == QJsonParseError::NoError) // 没有出错
    {
        ...
        if( doc.isObject()) // 如果对象不为空
        {
            ...
            for(int i = 0; i < list.size(); ++i)
            {
                if( list.at(i) == key )
                {
                    cout<<tmp.value( list.at(i) ).toString()<<endl;
...
// 登录信息，写入配置文件
void Common::writeLoginInfo(QString user, QString pwd,QString server_IP,QString server_port, bool isRemeber, QString path)
{
    // web_server信息
